...
contract GasEfficientVoting{
    ...
    uint8 public proposalCount; // most 256 proposals
    struct Proposal{
        bytes32 name;
        uint32 voteCount;
        uint32 startTime;
    ...
    function vote(uint8 proposalId) external{
        ...
        require(currentTime >= proposals[proposalId].startTime, "Voting has not satrted");
    ...
    function executeProposal(uint8 proposalId) external{
        require(proposalId < proposalCount, "Invalid Proposal");
        require(block.timestamp < proposals[proposalId].endTime, "Voting period not over");
